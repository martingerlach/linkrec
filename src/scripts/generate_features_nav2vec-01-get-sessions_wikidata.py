...
def main():
    ...
    path_save = os.path.abspath("../../data/%s/" % wiki_id)
    # filename_save = '%s.reading-sessions-%s--%s'%(lang,date_start_str,date_end_str)
    filename_save = "%s.reading-sessions" % (wiki_id)
    ...
    if wiki_id == "wikidata":
        ...
        df_actor = df_actor.where(F.col("wiki_db") == wiki_id)
    ...
    if wiki_id == "wikidata":
        ...
        df_wd = df_wd.where(F.col("wiki_db") == wiki_id)
