...
df_wd = (
    ...
    .where(F.col('snapshot') == snapshot) ## resolve issues with non-mathcing wikidata-items
...
df_statements = (
    ...
    .withColumn(
        "statement_value",
        F.when(
            (F.col("statement_property").isin(list_properties_keep))&(F.col("statement_value_type")=="wikibase-item"),
...
## aggregate all statemene-values (qids) for a given page-id
df_wdproperties = (
    df_statements
    .groupby("page_id")
    .agg(
